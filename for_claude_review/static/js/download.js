...
export function openViewerUrl(url, title) {
  ...
  const f = $('viewerIframe');
  f.src = ext === 'pdf' ? url + '#toolbar=1&view=FitH' : url;
  f.style.display = 'block';
  // Remove any leftover pdf-object elements from previous openViewer calls
  $('viewerBody').querySelectorAll('.pdf-object').forEach(el => el.remove());
}
